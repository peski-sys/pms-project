...
import { fileSubmittedMigration } from "@/app/api/migrationAPI/actions";


export function UploadMigration({onUpload} : {onUpload?: () => void}) {
  ...
  const handleFileUpload = async () => {
    await fileSubmittedMigration(selectedFile);
    ...
    <Dialog>
      <form id="fileSubmission" action={handleFileUpload}>
        <DialogTrigger asChild>
          <Button variant="outline" size="default" className="font-bold">Migrate!</Button>
        </DialogTrigger>
  ...
  <div className="flex flex-col items-center space-y-2">
      <div className="flex flex-col items-center space-y-2" key={selectedFile.name}>
  ...
  <div className="flex flex-col items-center space-y-2">
    ...
    <p className="text-xs text-gray-500">or</p>
            ...
            {/* File Info */}
            {selectedFile && (
              <div className="text-xs text-gray-500 text-center" key={selectedFile.name}>
